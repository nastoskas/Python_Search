...
def move(ghost,potez):
    ...
    return ghost[0]+moves[potez][0], ghost[1]+moves[potez][1]
...
class GhostOnSkates(Problem):
    ...
    def successor(self, state):
        ...
        x_ghost = state[0]
        y_ghost = state[1]
        if self.check_valid((x_ghost,y_ghost+1),walls,n):
            successors["Gore 1"] = (x_ghost,y_ghost+1)
        if self.check_valid((x_ghost,y_ghost+2),walls,n):
            successors["Gore 2"] = (x_ghost,y_ghost+2)
        if self.check_valid((x_ghost,y_ghost+3),walls,n):
            successors["Gore 3"] = (x_ghost,y_ghost+3)
        if self.check_valid((x_ghost+1,y_ghost),walls,n):
            successors["Desno 1"] = (x_ghost+1,y_ghost)
        if self.check_valid((x_ghost + 2, y_ghost), walls, n):
            successors["Desno 2"] = (x_ghost + 2, y_ghost)
        if self.check_valid((x_ghost + 3, y_ghost), walls, n):
            successors["Desno 3"] = (x_ghost + 3, y_ghost)
    ...
    def h(self, node):
        ...
        x_pac = self.goal[0]
        y_pac = self.goal[1]
        return (abs(x_ghost-x_pac) + abs(y_ghost-y_pac))/3
