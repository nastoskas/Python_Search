...
from towers import depth_first_graph_search
...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        # gore 1
        y_new = one[1] + 1
        if self.check_valid(((one[0], y_new), two, three, four, five)):
            successors["Pomesti kvadratche 1 gore"] = ((one[0], y_new), two, three, four, five)
        #levo 1
        x_new = one[0] - 1
        if self.check_valid(((x_new, one[1]), two, three, four, five)):
            successors["Pomesti kvadratche 1 levo"] = ((x_new, one[1]), two, three, four, five)

        # dolu 1
        y_new = one[1] - 1
        if self.check_valid(((one[0], y_new), two, three, four, five)):
            successors["Pomesti kvadratche 1 dolu"] = ((one[0], y_new), two, three, four, five)

        #desno 1
        x_new = one[0] + 1
        if self.check_valid(((x_new, one[1]), two, three, four, five)):
            successors["Pomesti kvadratche 1 desno"] = ((x_new, one[1]), two, three, four, five)

        #-------------------------------------------------------
        # gore 2
        y_new = two[1] + 1
        if self.check_valid((one, (two[0], y_new), three, four, five)):
            successors["Pomesti kvadratche 2 gore"] = (one, (two[0], y_new), three, four, five)
        # levo 2
        x_new = two[0] - 1
        if self.check_valid((one,(x_new, two[1]), three, four, five)):
            successors["Pomesti kvadratche 2 levo"] = (one,(x_new, two[1]), three, four, five)

        # dolu 2
        y_new = two[1] - 1
        if self.check_valid((one, (two[0], y_new), three, four, five)):
            successors["Pomesti kvadratche 2 dolu"] = (one, (two[0], y_new), three, four, five)
        # desno 2
        x_new = two[0] + 1
        if self.check_valid((one,(x_new, two[1]), three, four, five)):
            successors["Pomesti kvadratche 2 desno"] = (one,(x_new, two[1]), three, four, five)

        #------------------------------------------------------
        # gore 3
        y_new = three[1] + 1
        if self.check_valid((one, two, (three[0], y_new), four, five)):
            successors["Pomesti kvadratche 3 gore"] = (one, two, (three[0], y_new), four, five)
        # levo 3
        x_new = three[0] - 1
        if self.check_valid((one,two,(x_new, three[1]), four, five)):
            successors["Pomesti kvadratche 3 levo"] = (one,two,(x_new, three[1]), four, five)
        # dolu 3
        y_new = three[1] - 1
        if self.check_valid((one, two, (three[0], y_new), four, five)):
            successors["Pomesti kvadratche 3 dolu"] = (one, two, (three[0], y_new), four, five)
        # desno 3
        x_new = three[0] + 1
        if self.check_valid((one, two ,(x_new, three[1]), four, five)):
            successors["Pomesti kvadratche 3 desno"] = (one, two ,(x_new, three[1]), four, five)

        #----------------------------------------------------------
        # gore 4
        y_new = four[1] + 1
        if self.check_valid((one, two, three, (three[0], y_new), five)):
            successors["Pomesti kvadratche 4 gore"] = (one, two, three, (four[0], y_new), five)

        # levo 4
        x_new = four[0] - 1
        if self.check_valid((one, two,three ,(x_new, four[1]), five)):
            successors["Pomesti kvadratche 4 levo"] = (one, two,three ,(x_new, four[1]), five)

        # dolu 4
        y_new = four[1] - 1
        if self.check_valid((one, two,three ,(four[0], y_new), five)):
            successors["Pomesti kvadratche 4 dolu"] = (one, two,three ,(four[0], y_new), five)
        # desno 4
        x_new = four[0] + 1
        if self.check_valid((one, two,three ,(x_new, four[1]), five)):
            successors["Pomesti kvadratche 4 desno"] = (one, two, (x_new, three[1]), four, five)

        #-------------------------------------------------------
        # gore 5
        y_new = five[1] + 1
        if self.check_valid((one, two, three, four, (five[0], y_new))):
            successors["Pomesti kvadratche 5 gore"] = (one, two, three, four, (five[0], y_new))
        # levo 5
        x_new = five[0] - 1
        if self.check_valid((one, two,three,four ,(x_new, five[1]))):
            successors["Pomesti kvadratche 5 levo"] = (one, two,three,four,(x_new, five[1]))

        # dolu 5
        y_new = five[1] - 1
        if self.check_valid((one, two, three, four, (five[0], y_new))):
            successors["Pomesti kvadratche 5 dolu"] = (one, two, three, four, (five[0], y_new))

        # desno 5
        x_new = five[0] + 1
        if self.check_valid((one, two,three,four,(x_new, five[1]))):
                successors["Pomesti kvadratche 5 desno"] = (one, two,three,four,(x_new, five[1]))
