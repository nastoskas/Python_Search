...
class Towers(Problem):
    ...
    def generate_state(self, state, i, j):
        ...
        top = new_state[i].pop()
        new_state[j].append(top)

        return tuple(map(tuple, new_state))

    def successor(self, state):
        successors = dict()
        for i, pillar1 in enumerate(state):
            if len(pillar1)==0:
                continue
            top = pillar1[-1]

            for j, pillar2 in enumerate(state):
                if i == j:
                    continue
                if len(pillar2)==0 or top <= pillar2[-1]:
                    successors[f'MOVE TOP BLOCK FROM PILLAR {i + 1} TO PILLAR {j + 1}'] = self.generate_state(state, i, j)
...
if __name__ == "__main__":
    initial = tuple(map(tuple,input().replace(',','').split(';')))
    goal = tuple(map(tuple,input().replace(',','').split(';')))
    problem = Towers(initial, goal)
