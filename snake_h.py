...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        if valid_move(levo[0]):
            successors["SvrtiLevo"] = levo
        if valid_move(desno[0]):
            successors["SvrtiDesno"] = desno
        if valid_move(napred[0]):
    ...
    def h(self,node):
        snake, _, green = node.state
        head = snake[-1]
        max_apple_distance = 0
        result = 0
        for apple in green:
            if manhattan_distance(apple,head) > max_apple_distance:
                max_apple_distance = manhattan_distance(apple,head)
                result = max_apple_distance
        return result
...
if __name__ == "__main__":
    ...
    initiall = (tuple(snake_pos), 'd', tuple(zeleni_jabolki))
    start = Snake(initiall)

    if astar_search(start) is not None:
        print(astar_search(start).solution())
    else:
        print([])
